...
design.file <- paste(nf.dir,"design_new.csv",sep='');
...
deg.meta <- as.data.frame(cbind(sapply(res.list,function(x){sum(x$DEG)})))
...
ht <- Heatmap(heat.dat,
              ...
              # top_annotation          = ha1,
              # bottom_annotation       = anno_bottom,
              col                     = colorRamp2(c(-4, 0, 4), rev(colorRampPalette(brewer.pal(9, "RdBu"))(3))));

ht
# groups <-  c(0, seq(4,64,12))
groups <- (c(0,15,27,38,48))
decorate_heatmap_body('zscore', c(for(d in c(0,4,8,12,15,19,23,27,31,35,38,42,45,48,52,56,60)){
  ifelse(d %in% groups, x <- 'solid', x <- 'dashed')
  grid.lines(x=c(d/60, d/60), y=c(0,1), gp= gpar(col = 'black', lty=x, lwd=2))},
  grid.lines(y=c(0,0), x=c(0,1), gp= gpar(col = 'black', lty='solid', lwd=2)),
  grid.lines(y=c(1,1), x=c(0,1), gp= gpar(col = 'black', lty='solid', lwd=2))), slice = 1)
```
...
rvs <- res.list[26:30]
rvs.deg <- lapply(rvs, function(df){
  df <- df[df$DEG, ]
  df
})
rvs.deg <- unique(unlist(sapply(rvs.deg, row.names)))
rvs.deg

heat.dat   <- as.matrix(assay(vsd)[rvs.deg,]);
heat.dat   <- t(apply(heat.dat,1,function(x){((x-mean(x,na.rm=TRUE))/sd(x,na.rm=TRUE))}));
rownames(heat.dat) <- gtf.dat[rvs.deg,"gene_name"];
...
ht <- Heatmap(heat.dat,
              ...
              # top_annotation          = ha1,
              # bottom_annotation       = anno_bottom,
              col                     = colorRamp2(c(-3, 0, 5), rev(colorRampPalette(brewer.pal(9, "RdBu"))(3))));
min(heat.dat)
ht
# groups <-  c(0, seq(4,64,12))
groups <- (c(0,15,27,38,48,60))
decorate_heatmap_body('zscore', c(for(d in c(0,4,8,12,15,19,23,27,31,35,38,42,45,48,52,56,60)){
  ifelse(d %in% groups, x <- 'solid', x <- 'dashed')
  grid.lines(x=c(d/60, d/60), y=c(0,1), gp= gpar(col = 'black', lty=x, lwd=2))},
  grid.lines(y=c(0,0), x=c(0,1), gp= gpar(col = 'black', lty='solid', lwd=2)),
...
#     grid.lines(x=c(d/, d/64), y=c(0,1), gp= gpar(col = 'black', lty='solid', lwd=2))
