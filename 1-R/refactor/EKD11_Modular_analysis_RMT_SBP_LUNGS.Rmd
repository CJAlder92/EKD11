...
I have used the Singhania/O'Garra LUNG modules, using Qusage to quantify the relative encrihment of the modules.
...
### Revisions
I have changed the filtering process from P value < 0.05 to FDR < 0.1. 
...
module.anno <- read.csv(file=paste(data.dir, '/modular_analysis/Annotation of Lung modules.csv', sep=''),
                        header=TRUE,
                        sep=',');
module.dat <- read.csv(file=paste(data.dir, '/modular_analysis/Lung modules.csv', sep=''),
                       header=TRUE,
                       sep=',');

count.dat <- read.table(file= paste(results.dir, 'sbp_rmt.vst_counts_with_controls.xls.gz', sep=''),
                       header=TRUE,
                       sep='\t');
...
if(file.exists(paste(r.dir, "Paper/qusage_lung_results.Rdata", sep = ""))){
  load(paste(r.dir, "Paper/qusage_lung_results.Rdata", sep = ""))
}
...
if(!exists("lung_results.svn")){
  lung_results.svn <- lapply(c(1,2,3,4,6), function(d){
  ...
  names(lung_results.svn) <- c("Day1", "Day2", "Day3", "Day4", "Day6")
}

qusage.svn <- lapply(names(lung_results.svn), function(x){
  df <- qsTable(results.svn[[x]], number = 38)
  df$log.fold.change <- ifelse(df$FDR < 0.1, df$log.fold.change, NA)
  df <- df[ ,c(1,2)]
...
if(!exists("lung_results.rvn")){
  lung_results.rvn <- lapply(c(1,2,3,4,6), function(d){
  ...
  names(lung_results.rvn) <- c("Day1", "Day2", "Day3", "Day4", "Day6")
}

qusage.rvn <- lapply(names(lung_results.rvn), function(x){
  df <- qsTable(lung_results.rvn[[x]], number = 38)
  df$log.fold.change <- ifelse(df$FDR < 0.1, df$log.fold.change, NA)
  df <- df[ ,c(1,2)]
  colnames(df) <- c("Module", paste(x, "_FC", sep = ""))
  df
...
x <- ggplot(qusage.rvs.melt, aes(x = variable, y = reorder(Module, desc(Module)))) + 
  ...
  scale_color_gradientn(colours = rev(brewer.pal(11, "RdBu")),values = rescale(c(-0.6,0,1.1)), limits = c(-0.6,1.1), na.value = "transparent") +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
  panel.background = element_rect(fill = 'white'),
  plot.background = element_blank(),
...
x <- ggplot(qusage.all.fig, aes(x = transmission, y = reorder(Module, desc(Module)))) + 
  ...
  scale_color_gradientn(colours = rev(brewer.pal(11, "RdBu")),values = rescale(c(-0.6,0,1.1)), limits = c(-0.6,1.1)) +
  scale_size_area(max_size = 9) +
...
```{r save}
if(!file.exists(paste(r.dir, "Paper/lung_qusage_results.Rdata", sep = ""))){
save(lung_results.svn, lung_results.rvn, file = paste(r.dir, "Paper/lung_qusage_results.Rdata", sep = ""))
}
```


```{r}
module.sig <- grep(pattern = "B4\\ |B6|B7|B11|B14|B17|B18|B21|B24|B27|B31|B37", x = names(module.geneset))
...
module.sig.df <- lapply(module.sig.list, function(x){
  df <- data.frame(gene_id = x,
                   gene_name = gtf.dat[x, "gene_name"],
                   rmt_day4 = res.list[[4]][x, "DEG"],
                   rmt_day6 = res.list[[5]][x, "DEG"],
                   sbp_day4 = res.list[[9]][x, "DEG"],
                   sbp_day6 = res.list[[10]][x, "DEG"])
})
