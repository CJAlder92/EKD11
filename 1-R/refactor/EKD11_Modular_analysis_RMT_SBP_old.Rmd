...
for(n in 1:12){
  ...
  file_n <- gsub("/", "_", name)
  if (n %in% c(8,9,10)){
    tbl <- module.sig.df[[n]] %>% subset(sbp_day4 == T)
  }else{
    tbl <- module.sig.df[[n]] %>% subset(rmt_day4 == T)
  }
  write.table(tbl[,c(1,2)], file = paste(results.dir, "Paper/", file_n, "_sig_genes_2.tsv",sep= ""), sep = "\t", row.names = FALSE, quote = FALSE)
}
